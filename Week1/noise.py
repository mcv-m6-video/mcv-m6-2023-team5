    ...
    for ind in imageIds:
        ...
        if generation: # Generate new bbox
            for obj in annot[ind]:
                ...
                conf_score = np.round(np.random.uniform(0,1),3) # Random uniform confidence
            
                rand_bboxes = {'name': obj['name'], 'bbox': [xmin, ymin, xmax, ymax], 'confidence': conf_score}
                new_bboxes.append(rand_bboxes)
